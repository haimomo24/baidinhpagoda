  ...
  return (
     <div className="max-w-6xl mt-12 mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <article className="lg:col-span-2 grid gap-4">
        <h1 className="text-4xl font-bold leading-tight">{blog.name_en}</h1>

        {blog.title_1 && (
          <p className="text-lg leading-7 text-justify">{blog.title_1_en}</p>
        )}
        {blog.images_1 && (
          <img
            src={blog.images_1}
        ...
        {blog.title_2 && (
          <p className="text-lg leading-7 text-justify">{blog.title_2_en}</p>
        )}
        {blog.images_2 && (
          <img
            src={blog.images_2}
        ...
        {blog.title_3 && (
          <p className="text-lg leading-7 text-justify">{blog.title_3_en}</p>
        )}
      </article>

      {/* Bài viết liên quan */}
      <aside className="bg-gray-50 p-4 rounded-xl shadow-sm">
        <h2 className="text-lg font-semibold border-b pb-2 mb-4">Related articles</h2>
            ...
            <li key={item.id}>
              <Link href={`/vi/blog/${item.id}`} className="flex gap-3 items-stretch">
                ...
                <div className="flex flex-col justify-between h-full">
                  ...
                  {item.title_1 && (
                    <p className="text-xs text-gray-600 line-clamp-2 mt-1">{item.title_1_en}</p>
                  )}
                </div>
              </Link>
            </li>
